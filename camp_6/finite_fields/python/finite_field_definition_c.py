...
class FieldElement:
    ...
    def inverse(self):
        if self.value == 0:
            ...
            raise ValueError(error)
        t, new_t = 0, 1
        r_order, new_r_value = self.order, self.value
        # While the value is not 0
        while new_r_value != 0: # 3
            # Perform a simple division, e.g. F_10{3}
            quotient = r_order // new_r_value # 10//3 = 3
            t, new_t = new_t, (t - (quotient * new_t)) # 1, (0 - (3 * 1)) = 1, -3
            r_order, new_r_value = new_r_value, r_order - quotient * new_r_value # 3, 10 - (3 * 3) = 3, 1
        assert r_order == 1
        print(t, r_order)
        return self.__class__(value=t, order=self.order)

...
= Primer
:navtitle: Primer: The need for StarkNet
...
link:./bitcoin/block_verification/go[bitcoin/block_verification/go]
...
_blockchains are forced to make trade-offs that prevent them from being decentralized, scalable, and secure._
...
=== xref:./bitcoin/README.adoc[Bitcoin]
...
The Bitcoin nodes themselves listen for and link:./bitcoin/block_verifcation[validate] blocks of transactions that are broadcast to the network by the miner of that block.
They form a data structure called a Merkle Tree to obtain a root hash corresponding to all the transactions (and their order) in that block.
If one tx changes by even a single bit the Merkle root will be completely different.
